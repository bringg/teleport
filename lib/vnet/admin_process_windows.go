...
func runWindowsAdminProcess(ctx context.Context, cfg *windowsAdminProcessConfig) error {
	...
	clt, err := newUserProcessServiceClient(ctx, cfg.userProcessServiceAddr)
	if err != nil {
...
func newWindowsNetworkStackConfig(tun tunDevice, clt *userProcessServiceClient) (*networkStackConfig, error) {
	appResolver := newTCPAppResolver(clt, clockwork.NewRealClock())
...
func authenticateUserProcess(ctx context.Context, clt *userProcessServiceClient) error {
