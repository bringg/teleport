...
func (p *echoAppProvider) ResolveAppInfo(ctx context.Context, fqdn string) (*vnetv1.AppInfo, error) {
	return ResolveAppInfo(ctx, p, fqdn)
}
...
func (p *echoAppProvider) GetCachedClusterConfig(ctx context.Context, clt ClusterClient) (*ClusterConfig, error) {
	return p.clusterConfigCache.GetClusterConfig(ctx, clt)
}

func (p *echoAppProvider) ReissueAppCert(ctx context.Context, appInfo *vnetv1.AppInfo, targetPort uint16) (tls.Certificate, error) {
...
func (p *echoAppProvider) GetDialOptions(ctx context.Context, profileName string) (*vnetv1.DialOptions, error) {
...
func (p *echoAppProvider) OnNewConnection(_ context.Context, _ *vnetv1.AppKey) error {
	p.onNewConnectionCallCount.Add(1)
	return nil
}

func (p *echoAppProvider) OnInvalidLocalPort(_ context.Context, _ *vnetv1.AppInfo, _ *proto.RouteToApp) {
			...
			t.Run(fmt.Sprint(i), func(t *testing.T) {
				for _, tc := range validTestCases {
					t.Run(tc.app, func(t *testing.T) {
						...
						for _, requestedRouteToApp := range appProvider.RequestedRouteToApps(tc.app) {
...
func TestRemoteAppProvider(t *testing.T) {
	...
	localAppProvider := newEchoAppProvider(map[string]testClusterSpec{
		"root.example.com": {
			apps: []appSpec{
	...
	svc := newUserProcessService(localAppProvider)
	...
	remoteAppProvider, err := newUserProcessServiceClient(ctx, listener.Addr().String())
	require.NoError(t, err)
