...
func (a *App) run(ctx context.Context) error {
	...
	log := logger.Get(ctx)
	log.InfoContext(ctx, "Starting Teleport Access incident.io Plugin")
...
func (a *App) handleAcessRequest(ctx context.Context, event types.Event) error {
	...
	case types.OpPut:
		...
		// Log all fields of req for debugging
		log.With("request_details", fmt.Sprintf("%+v", reflect.ValueOf(req).Elem().Interface())).Debug("Request details")
		...
		default:
			log.With("event", event).Warn("Unknown request state")
			return nil
		}

		if err != nil {
			log.With(err).Error("Failed to process request")
			return trace.Wrap(err)
		}
	...
	case types.OpDelete:
		...
		if err := a.onDeletedRequest(ctx, reqID); err != nil {
			log.With(err).Error("Failed to process deleted request")
			return trace.Wrap(err)
		}
		return nil
	default:
...
func (a *App) getNotifySchedulesAndTeams(ctx context.Context, req types.AccessRequest) (schedules []string, teams []string, err error) {
	...
	if err != nil {
		log.DebugContext(ctx, fmt.Sprintf("No schedules to notify in %s", scheduleAnnotationKey))
	}
...
func (a *App) tryNotifyService(ctx context.Context, req types.AccessRequest) (bool, error) {
	...
	if err != nil {
		log.DebugContext(ctx, fmt.Sprintf("Skipping the notification: %s", err))
...
func (a *App) createAlert(ctx context.Context, reqID string, reqData RequestData) error {
	...
	ctx, log := logger.With(ctx, "incident_deduplication_key", data.DeduplicationKey)
...
func (a *App) tryApproveRequest(ctx context.Context, req types.AccessRequest) error {
	...
	if err != nil {
		logger.Get(ctx).DebugContext(ctx, fmt.Sprintf("Skipping the approval: %s", err))
		return nil
	}
...
func (a *App) resolveAlert(ctx context.Context, reqID string, resolution Resolution) error {
	...
	if !ok {
		logger.Get(ctx).DebugContext(ctx, fmt.Sprintf("Failed to resolve the alert: plugin data is missing"))
		return nil
	}

	ctx, log := logger.With(ctx, "incidentio_deduplication_key", alertID)
